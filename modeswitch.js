...
function enable90sMode() {
    ...
    * {
    ...
    .wiki-filters, .wiki-section-hero {
        background: #c0c0c0 !important;
    ...
    .wiki-filters button,
    .wiki-filters input[type="submit"],
    .wiki-filters input[type="button"] {
        background: #c0c0c0 !important;
        color: black !important;
    ...
    .wiki-filters button:active,
    .wiki-filters input[type="submit"]:active,
    .wiki-filters input[type="button"]:active {
        border-top: 2px solid #808080 !important;
        border-left: 2px solid #808080 !important;
        border-right: 2px solid #ffffff !important;
        border-bottom: 2px solid #ffffff !important;
  ...
  `;
    ...
    marquee.className = "ecl-90s-marquee";
    marquee.textContent =
        "★★★ WELCOME TO THE OFFICIAL ECL EXILES INFORMATION SUPERHIGHWAY ★★★ | UNDER CONSTRUCTION | BEST VIEWED IN NETSCAPE NAVIGATOR 4.0 YOU ARE VISITOR #000042 | SIGN OUR GUESTBOOK";
    ...
    setInterval(() => {
        marquee.style.background =
            marquee.style.background === "yellow" ? "lime" : "yellow";

        marquee.style.color =
            marquee.style.color === "red" ? "blue" : "red";
    }, 500);
...
`;
    ...
    document.title = "ECL Exiles Information Retrieval System v1.34.6";

    // Replace visible occurrences of the site title
    document.querySelectorAll("h1, .logo, .site-title, .navbar-brand").forEach(el => {
        el.textContent = "ECL Exiles - Information Retrieval System - v1.34.6";
    });

    const versionBanner = document.createElement("div");

    versionBanner.innerHTML = `
    <b>ECL EXILES INFORMATION RETRIEVAL SYSTEM</b><br>
    Version 1.34.6 (Production)<br>
...
`;
    ...
    document.body.prepend(versionBanner);

    const panicButton = document.createElement("button");

    panicButton.id = "take-me-back-button";
    panicButton.textContent = "OH GOD TAKE ME BACK";
...
`;
    ...
    document.addEventListener("mousemove", (e) => {
        ...
        const rect = panicButton.getBoundingClientRect();
        ...
        if (distance < 200 && !fleeing) {

            fleeing = true;

            const fleeDistance = 250;

            const angle = Math.atan2(dy, dx);

            let newX =
                centreX - Math.cos(angle) * fleeDistance;

            let newY =
                centreY - Math.sin(angle) * fleeDistance;

            const margin = 20;

            newX = Math.max(
                margin,
                Math.min(window.innerWidth - rect.width - margin, newX)
            );

            newY = Math.max(
                margin,
                Math.min(window.innerHeight - rect.height - margin, newY)
            );

            panicButton.style.left = `${newX}px`;
            panicButton.style.top = `${newY}px`;
            panicButton.style.transform = "none";
    ...
    panicButton.addEventListener("mouseenter", () => {
    ...
    panicButton.addEventListener("click", () => {
        alert(
            "ERROR 0x80004005\n\nUnable to return to the future.\n\nPlease upgrade to Internet Explorer 4.0."
        );
    });
...
(function () {
    ...
    if (document.getElementById("enhancedGraphicsButton")) {
        return;
    }

    const button = document.createElement("button");

    button.id = "enhancedGraphicsButton";
    button.textContent = "Click Here For Enhanced Graphics™";
    ...
    button.addEventListener("click", () => {
        ...
        if (typeof enable90sMode === "function") {

            enable90sMode();

            button.textContent = "Enhanced Graphics Enabled";
            ...
            alert(
                "ERROR:\nEnhanced Graphics Module Not Installed.\nPlease upgrade to Netscape Navigator 4.0."
            );
